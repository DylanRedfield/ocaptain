...
class ExtendedDucklingHTTPExtractor(EntityExtractor):
    ...
    def __init__(self, component_config=None, language=None):
        ...
        super(ExtendedDucklingHTTPExtractor, self).__init__(component_config)
    ...
    def modify_grain(self, match):
        values = match['additional_info']['values']

        is_period = False
        #2019-01-09T18:00:00.000+00:00
        for i, value in enumerate(values):
            time = parser.parse(value['value'])
            for j in range(i + 1, len(values)):
                sub_time = parser.parse(values[j]['value'])

                if time.year == sub_time.year and time.month == sub_time.month and time.day == sub_time.day and time.hour != sub_time.hour and time.minute == sub_time.minute:
                    is_period = True
        ...
        if self._url() is not None:
            ...
            if message.get('ordinals'):
                ...
                for ordinal in ordinals:
                    relevant_matches.extend(self._duckling_parse(ordinal, reference_time))

            extracted = convert_duckling_format_to_rasa(relevant_matches)
            for match in extracted:
                if match['entity'] == 'time':
                    self.modify_grain(match)
        else:
            extracted = []
